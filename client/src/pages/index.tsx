...
export default function Orders() {
  ...
  const { control, handleSubmit, reset } = useForm<z.infer<typeof order>>({
  ...
  const orders = useQuery({
    ...
    queryFn: () =>
      getRecords<z.infer<typeof order>>({
        pathname: '/order',
        params: {
          pageSize,
          page,
  ...
  return (
    <>
      <div className="flex flex-col gap-5 lg:gap-10">
        ...
        <div className="flex flex-col items-center justify-between gap-5 lg:flex-row-reverse">
          ...
          {orders.data && (
            <span className="self-start font-bold">
              Всего заказов: {orders.data?.total}
            </span>
          )}
        </div>
        {orders.data && (
        ...
        {orders.data && orders.data.total > pageSize && (
